...
# Check if the data frame 'reg_dfw' is correctly loaded and contains the necessary columns
print("Data frame 'reg_dfw' contains the following columns:")
print(colnames(reg_dfw))  # This will list all column names in your data frame

# Check if 'yield_mgha' exists in the data frame
if ("yield_mgha" %in% names(reg_dfw)) {
  # Setting contrasts for categorical variables (if any)
  options(contrasts = c("contr.sum", "contr.poly"))

  # Fitting the model, specify the data frame in the model function
  mod3_quad <- lmer(yield_mgha ~ (1|rep) + sr_ksha + I(sr_ksha^2), data = reg_dfw)

  # Output the summary of the model
  print(summary(mod3_quad))
} else {
  cat("Error: 'yield_mgha' not found in your data frame 'reg_dfw'\n")
}
...
# Assuming 'nd' dataframe has the predictions already added as columns: mod1_yield_mgha, mod2_yield_mgha, etc.
# Ensure 'nd' has the same 'sr_ksha' values as 'reg_dfw' or join/merge them appropriately if necessary

# Create a single ggplot object and add multiple layers
ggplot(reg_dfw, aes(x = sr_ksha, y = yield_mgha)) +
  geom_point(size = 4, alpha = 0.6) +  # Base layer of points from reg_dfw
  geom_line(data = nd, aes(y = mod1_yield_mgha), color = "forestgreen", linewidth = 1.5) +  # Line for first model prediction
  geom_line(data = nd, aes(y = mod2_yield_mgha), color = "blue", linewidth = 1.5) +  # Line for second model prediction
  geom_line(data = nd, aes(y = mod3_yield_mgha), color = "purple", linewidth = 1.5) +  # Line for third model prediction
  geom_line(data = nd, aes(y = mod4_yield_mgha), color = "red", linewidth = 1.5)  # Line for fourth model prediction
